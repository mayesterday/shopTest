  ...
  <div class="auth-wrapper auth-v2">
    <b-row class="auth-inner m-0">
      ...
      <b-link class="brand-logo">
        <vuexy-logo />
        <h2 class="brand-text text-primary ml-1">Vuexy1</h2>
      </b-link>
      <!-- /Brand logo-->
  ...
  methods: {
    ...
    async login() {
      try {
        ...
        const resp = await useJwt.login({
          username: this.userEmail,
          password: this.password,
        })
        const { token, name } = resp.data.data
        // FIXME: หน้าบ้านต้องส่งมา
        const userData = {
          role: 'application', // src\auth\utils.js
          fullName: name,
          username: name,
          ability: [{ action: 'application', subject: 'all' }], // ไม่มี การทำงาน
        }
